...
5. `dog_behavior` 通过行为树导航叶子直接下发导航目标到 Nav2 action `/navigate_to_pose`。
6. `dog_behavior` 同时发布执行状态到 `/behavior/nav_exec_state` 供观测与联调。
...
- `dog_behavior` -> Nav2: `nav2_msgs/action/NavigateToPose` (`/navigate_to_pose`)
- `dog_behavior` -> 状态观测: `/behavior/nav_exec_state` (`std_msgs/msg/String`)

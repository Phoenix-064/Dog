...
- Build `${PROJECT_NAME}_lib` for core logic and `${PROJECT_NAME}_bt_node` as entrypoint executable.
...
- When testing actions, mirror the existing mock action server pattern used in `src/dog_behavior/test/test_execute_behavior_action.cpp` and `src/dog_behavior/test/test_navigate_to_pose_action.cpp`.
